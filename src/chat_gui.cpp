...
ChatBotFrame::ChatBotFrame(const wxString &title) : wxFrame(NULL, wxID_ANY, title, wxDefaultPosition, wxSize(width, height))
{
    ...
    wxFont font = _userTextCtrl->GetFont();
...
void ChatBotFrame::OnEnter(wxCommandEvent &WXUNUSED(event))
{
    ...
    for(std::string answer : _panelDialog->GetChatBot()->getAnswer()){
      _panelDialog->PrintChatbotResponse(answer);
    }
}
    ...
    : wxScrolledWindow(parent, id)
{
    ...
    // print first dialog
    for(std::string answer : _chatBot->getAnswer()) {
      PrintChatbotResponse(answer);
    }
...
{
    wxImage image;
    image.LoadFile(imgBasePath + "inner.jpg");
    ...
    : wxPanel(parent, -1, wxPoint(-1, -1), wxSize(-1, -1), wxBORDER_NONE)
{
    ...
    wxBitmap *bitmap = new wxBitmap(imgBasePath + "chatbot.png", wxBITMAP_TYPE_PNG);
    ...
    this->SetBackgroundColour((isFromUser == true ? wxColour(71, 140, 118) : wxColour(41, 52, 100)));
}
